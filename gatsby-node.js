...
exports.sourceNodes = async ({
  ...
  if (validationErrors.length > 0) console.error(`Plugin Error - See below message${validationErrors.length > 1 ? 's' : ''}\n${validationErrors.map(validationMessageReport).join('\n')}`);
  if (validationWarnnings.length > 0) console.warn(`Plugin Warmings - See below message${validationWarnnings.length > 1 ? 's' : ''}\n${validationWarnnings.map(validationMessageReport).join('\n')}`);
  const params = Object.assign({}, defaultOptions, {}, options);

  if (!validationErrors.length > 0) {
    const api = apiHandler.default({
      actions,
      store,
      cache,
      createNodeId,
